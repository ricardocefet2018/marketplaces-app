...
export class DB {
  ...
  static async start(): Promise<void> {
    if (this.instance?.dataSource.isInitialized) return;
  ...
  private static async initializeDatabase(): Promise<void> {
    try {
      ...
      if (process.env.NODE_ENV === "test") await this.clearDatabase();
  ...
  private static async setupDefaultSettings(): Promise<void> {
    ...
    if (existingSettings.length === 0) {
      ...
      infoLogger("Settings already exist in the database.");
    }
  }
}
