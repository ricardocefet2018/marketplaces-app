...
export class TradeManager extends EventEmitter {
    ...
    public async createTradeForMarketcsgo(data: MarketcsgoTradeOfferPayload) {
        ...
        if (this._user.marketcsgo.dontSentTrades.includes(data.hash)) return;
    ...
    private async createTrade(
        ...
        try {
            ...
            if (!itemsToSend.every((v) => typeof v != "undefined")) {
                if (marketplace === "MarketCSGO") {
                    this._user.marketcsgo.dontSentTrades.push(id.toString());
                    await this._user.save();
                }
                this.infoLogger(
            ...
            if (alreadyInTrade) {
                if (marketplace === "MarketCSGO") {
                    const dontSend = this._user.marketcsgo.dontSentTrades.includes(id.toString());
                    if (!dontSend) {
                        this._user.marketcsgo.dontSentTrades.push(id.toString());
                        await this._user.save();
                    }
